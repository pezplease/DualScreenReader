...
bool Epub::parse_content_opf(ZipFile &zip, std::string &content_opf_file)
{
  ...
  while (item)
  {
    ...
    // grab the ncx file — match by id or by media-type (EPUB3 may use a non-"ncx" id)
    const char *media_type_attr = item->Attribute("media-type");
    if (item_id == "ncx" ||
        (media_type_attr && strcmp(media_type_attr, "application/x-dtbncx+xml") == 0))
    {
      m_toc_ncx_item = href;
    }
    items[item_id] = href;
...
bool Epub::parse_toc_ncx_file(ZipFile &zip)
{
  ...
  std::string ncx_path = normalise_path(m_toc_ncx_item);
  char *ncx_data = (char *)zip.read_file_to_memory(ncx_path.c_str());
  if (!ncx_data)
  {
    ESP_LOGE(TAG, "Could not find %s", ncx_path.c_str());
    return false;
  }
  ...
  while (navPoint)
  {
    auto navLabelEl = navPoint->FirstChildElement("navLabel");
    auto textEl = navLabelEl ? navLabelEl->FirstChildElement("text") : nullptr;
    auto textNode = textEl ? textEl->FirstChild() : nullptr;
    auto content = navPoint->FirstChildElement("content");
    if (!textNode || !content || !content->Attribute("src"))
    {
...
bool Epub::load()
{
  ...
  {
    // NCX is absent or unparseable (common in EPUB3 books).
    // The book is still readable via its spine; TOC will be empty.
    ESP_LOGW(TAG, "No table of contents — navigating by spine only");
  }
  return true;
}
